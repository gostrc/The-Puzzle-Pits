...
main()
{
  ...
  do{
    ...
    ShadowText(141,156,"Start New Game",14,1);
    ShadowText(141,164,"Quit",4,1);
    FadeIn();
    CheckMouse();
    inkey=0xff;
    menuitem=0;
    ...
    while(notdone){
      ...
      ScreenSwap();
      if(choice>=0) notdone=0;
      oldmenuitem=menuitem;
      while(oldmenuitem==menuitem && notdone){
        CheckMouse();
        switch(inkey){
          case 13:  choice=menuitem; oldmenuitem=-1; break;
          case 28:
            inkey=0xff;
            menuitem--;
          ...
          case 29:
            ...
            break;
          case 'e':  menuitem=0; oldmenuitem=-1; choice=0; break;
          case 's':  menuitem=1; oldmenuitem=-1; choice=1; break;
          case 'q':  menuitem=2; oldmenuitem=-1; choice=2; break;
        }
      }
    }
    inkey=0xff;
    switch(choice){
      case 0:
        ...
        if(x==0){
          ...
          x=0;
          do{
            GetLevelName(gamevars.level,x);
            if(x==0) x=1;
            if(gamevars.level[0]!=0){
              strcpy(fname,LEVELPATH);
              strcat(fname,gamevars.level);
              strcat(fname,".pit");
              fp=ReadFile(fname);
              if(ReadWord(fp)==0x0895) ReadString(fp,fname);
              CloseFile(fp);
            }
          }while(stricmp(password,fname)!=0 && gamevars.level[0]!=0);
          IncrementLevel();
        }
        if(gamevars.level[0]!=0){
          Clear();
